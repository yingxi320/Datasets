...
def train(mnist):
    ...
    cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(tf.argmax(y, 1), tf.argmax(y_, 1))

    #计算当前bathc中所有样例的交叉熵平均值
    cross_entropy_mean = tf.reduce_mean(cross_entropy)
    accuracy_y = inference(x, None, True)
    ...
    correct_prediction = tf.equal(tf.argmax(accuracy_y, 1), tf.argmax(y_, 1))
